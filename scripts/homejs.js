...
function GenerateExcercise(event){
    ...
    //eseguo il fetch
    fetch("generate_exercise.php").then(onResponseExercise).then(onJsonExercise);
}
...
function SearchMusic(event){
    ...
    fetch("spotify_track.php?track=" + encodeURIComponent(titolo)).then(onResponseMusic).then(onJsonMusic);
}
...
function load_preferiti(){
    fetch("load_preferiti.php").then(OnResponsePreferiti).then(OnJsonPreferiti)
}
...
function AppendComment(event){ 
    ...
    fetch("carica_commento.php?text=" + encodeURIComponent(commento)).then(onResponseComments).then(onJsonAppendComment);
}
...
function load_comments(){
    fetch("load_comments.php").then(onResponseComments).then(onJsonComments);
}
